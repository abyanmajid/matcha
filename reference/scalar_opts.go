...
type Options struct {
	...
	Source             string              `json:"Source,omitempty"` // allow external URL ou local path file
...
func defaultOptions(option Options) *Options {
	...
	if returnOptions.PageTitle == "" {
		returnOptions.PageTitle = "API Reference"
	}

	if !returnOptions.IsEditable {
		returnOptions.IsEditable = false
	}

	if !returnOptions.ShowSidebar {
		returnOptions.ShowSidebar = true
	}

	if !returnOptions.HideModels {
		returnOptions.HideModels = false
	}

	if !returnOptions.HideDownloadButton {
		returnOptions.HideDownloadButton = false
	}

	if !returnOptions.DarkMode {
		returnOptions.DarkMode = true
	}

	if returnOptions.SearchHotKey == "" {
		returnOptions.SearchHotKey = "/"
	}
